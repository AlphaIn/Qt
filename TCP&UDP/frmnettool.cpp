...
void frmNetTool::initForm()
{
    ...
    ui->btnSendUdpClient->setEnabled(false);
    ui->txtDataUdpClient->setReadOnly(true);
    ...
    udpClient->abort();
    connect(udpClient, SIGNAL(readyRead()), this, SLOT(udpClientReadData()));
    connect(udpClient, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(udpClientReadError()));
    ...
    timerUdpClient = new QTimer(this);
    timerUdpClient->setInterval(App::IntervalUdpClient);
    connect(timerUdpClient, SIGNAL(timeout()), this, SLOT(sendDataUdpClient()));
    ...
    connect(ui->btnSendUdpClient, SIGNAL(clicked()), this, SLOT(sendDataUdpClient()));
}
...
void frmNetTool::initConfig()
{
    ...
    ui->cboxIntervalUdpClient->addItems(interval);
    ...
    ui->frmUdpClient->setMinimumWidth(App::RightPanelWidth);
    ui->frmUdpClient->setMaximumWidth(App::RightPanelWidth);
    ...
    //Udp client部分
    ui->ckHexSendUdpClient->setChecked(App::HexSendUdpClient);
    connect(ui->ckHexSendUdpClient, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->ckHexReceiveUdpClient->setChecked(App::HexReceiveUdpClient);
    connect(ui->ckHexReceiveUdpClient, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->ckDebugUdpClient->setChecked(App::DebugUdpClient);
    connect(ui->ckDebugUdpClient, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->ckAutoSendUdpClient->setChecked(App::AutoSendUdpClient);
    connect(ui->ckAutoSendUdpClient, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->cboxIntervalUdpClient->setEnabled(App::AutoSendUdpClient);
    ui->cboxIntervalUdpClient->setCurrentIndex(ui->cboxIntervalUdpClient->findText(QString::number(App::IntervalUdpClient)));
    connect(ui->cboxIntervalUdpClient, SIGNAL(currentIndexChanged(int)), this, SLOT(saveConfig()));

    ui->txtUdpServerIP->setText(App::UdpServerIP);
    qDebug() << App::UdpServerIP;
    connect(ui->txtUdpServerIP, SIGNAL(textChanged(QString)), this, SLOT(saveConfig()));

    ui->txtUdpServerPort->setText(QString::number(App::UdpServerPort));
    qDebug() << App::UdpServerPort;
    connect(ui->txtUdpServerPort, SIGNAL(textChanged(QString)), this, SLOT(saveConfig()));

    ui->txtUdpServerLocalPort->setText(QString::number(App::UdpServerLocalPort));
    connect(ui->txtUdpServerLocalPort,SIGNAL(textChanged(QString)), this, SLOT(saveConfig()));
}
...
void frmNetTool::saveConfig()
{
    ...
    App::HexSendUdpClient = ui->ckHexSendUdpClient->isChecked();
    App::HexReceiveUdpClient = ui->ckHexReceiveUdpClient->isChecked();
    App::DebugUdpClient = ui->ckDebugUdpClient->isChecked();

    bool autoSendUdpClient = ui->ckAutoSendUdpClient->isChecked();

    if (autoSendUdpClient != App::AutoSendUdpClient) {
        App::AutoSendUdpClient = autoSendUdpClient;
        ui->cboxIntervalUdpClient->setEnabled(App::AutoSendUdpClient);
    ...
    App::IntervalUdpClient = ui->cboxIntervalUdpClient->currentText().toInt();
    timerUdpClient->setInterval(App::IntervalUdpClient);
...
void frmNetTool::appendUdpClient(quint8 type, QString msg)
{
    if (countUdpClient > msgMaxCount) {
        ui->txtDataUdpClient->clear();
        countUdpClient = 0;
    }
    ...
    if (type == 0) {
        str = ">> 发送 :";
        ui->txtDataUdpClient->setTextColor(QColor("dodgerblue"));
    } else if (type == 1) {
        str = "<< 接收 :";
        ui->txtDataUdpClient->setTextColor(QColor("red"));
    }

    ui->txtDataUdpClient->append(QString("时间[%1] %2 %3").arg(TIMEMS).arg(str).arg(msg));
    countUdpClient++;
}
...
void frmNetTool::udpClientReadData()
{
    if (udpClient->bytesAvailable() <= 0) {
        return;
    }

    QByteArray data = udpClient->readAll();
    QString buffer;
...
void frmNetTool::udpClientReadError()
{
    udpClient->disconnectFromHost();
    ui->btnUdpConnect->setText("连接");
    ui->btnSendUdpClient->setEnabled(false);
    ui->cboxSendUdpClient->lineEdit()->clear();
    appendUdpClient(1, QString("连接服务器失败,原因 : %1").arg(udpClient->errorString()));
}
...
void frmNetTool::clientUdpReadData(int , QString ip, int port, QByteArray data)
{
    QString buffer;
    ...
    appendUdpServer(1, QString("%1 [%2:%3]").arg(buffer).arg(ip).arg(port));
    ...
    if (App::DebugUdpServer) {
        ...
        for (int i = 0; i < count; i++) {
            if (keys.at(i) == buffer) {
                sendDataUdpServer(ip, port, values.at(i));
                break;
            }
        }
    }
...
void frmNetTool::clientUdpConnect(int , QString ip, int port)
{
   /* appendUdpServer(1, QString("客户端[%1:%2] 上线").arg(ip).arg(port));

    ui->listUdpClient->clear();
    ui->listUdpClient->addItems(UdpServer->getClientInfo());
    ui->labUdpClientCount->setText(QString("已连接客户端共 %1 个").arg(UdpServer->getClientCount()));

    int count = ui->listUdpClient->count();

    if (count > 0) {
        ui->listUdpClient->setCurrentRow(count - 1);
    }*/
}

void frmNetTool::clientUdpDisConnect(int , QString ip, int port)
{
   /* appendUdpServer(1, QString("客户端[%1:%2] 下线").arg(ip).arg(port));
...
void frmNetTool::readSendData()
{
	...
	do {
		...
		if (line != "") {
            ...
            ui->cboxSendUdpClient->addItem(line);
...
void frmNetTool::sendDataUdpClient()
{
    QString data = ui->cboxSendUdpClient->currentText();
    sendDataUdpClient(data);
}

void frmNetTool::sendDataUdpClient(QString data)
{
    ...
    if (!ui->btnSendUdpClient->isEnabled()) {
        return;
    }
    ...
    //从界面上读取ip和端口
    const QString address_text=ui->txtUdpServerIP->text();
    const QHostAddress address=QHostAddress(address_text);
    const unsigned short port=ui->txtUdpServerPort->text().toUShort();
    //udpClient->write(buffer);
    udpClient->writeDatagram(buffer,address,port);
    appendUdpClient(0, data);
}
...
void frmNetTool::on_btnUdpConnect_clicked()
{
    if (ui->btnUdpConnect->text() == "连接") {
        udpClient->bind(QHostAddress(App::UdpServerIP), (quint16)(App::UdpServerPort));

        if (udpClient->state()==QAbstractSocket::BoundState/*tcpClient->waitForConnected(1000)*/) {
            ui->btnUdpConnect->setText("断开");
            ui->btnSendUdpClient->setEnabled(true);
            appendUdpClient(0, "连接服务器成功");
        } else {
            appendUdpClient(1, "连接服务器失败");
        }
    } else {
        udpClient->abort();

        if (udpClient->state() == QAbstractSocket::UnconnectedState || udpClient->waitForDisconnected(1000)) {
            ui->btnUdpConnect->setText("连接");
            ui->btnSendUdpClient->setEnabled(false);
            appendUdpClient(0, "断开连接成功");
        }
    }
}

void frmNetTool::on_btnClearUdpClient_clicked()
{
    ui->txtDataUdpClient->clear();
    countUdpClient = 0;
}
